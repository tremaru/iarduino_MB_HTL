...
float	iarduino_MB_HTL::getTEM(void){																						//
			...
			int32_t tem = objModbus->inputRegisterRead(valID, AI_TEM);														//	Читаем температуру из регистра "Input Register" AI[AI_TEM]. Функция возвращает значение, или -1 при неудаче.
...
float	iarduino_MB_HTL::getHUM(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			int32_t hum = objModbus->inputRegisterRead(valID, AI_HUM);														//	Читаем влажность из регистра "Input Register" AI[AI_HUM]. Функция возвращает значение, или -1 при неудаче.
...
float	iarduino_MB_HTL::getHUMA(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			int32_t hum = objModbus->inputRegisterRead(valID, AI_HUMA);														//	Читаем влажность из регистра "Input Register" AI[AI_HUMA]. Функция возвращает значение, или -1 при неудаче.
...
float	iarduino_MB_HTL::getLUX(void){																						//
			objModbus->codeError=ERROR_SYNTAX; if( !valID ){ return -1.0f; }												//	Ошибка синтаксиса, модуль не инициализирован.
			return (float)objModbus->inputRegisterRead(valID, AI_LUX);														//	Читаем значение из регистра "Input Register" AI[AI_LUX]. Функция возвращает значение, или -1 при неудаче.
}																															//
																															//
...
bool	iarduino_MB_HTL::setPeriod(uint8_t sensor, float period){															//	sensor - SENSOR_ALL/SHT/LTR, period - период опроса в секундах ( 0,1...6553,5с ).
			...
			if( sum==1 ){ return objModbus->holdingRegisterWrite(valID, adr, data); }										//	Записываем значение data в регистр "Holding Register" adr. Функция возвращает 1 при успехе, 0 при неудаче.
...
float	iarduino_MB_HTL::getPeriod(uint8_t sensor){																			//	sensor - SENSOR_SHT/LTR.
			...
			int32_t period = objModbus->holdingRegisterRead(valID, adr);													//	Читаем значение из регистра "holding Register" adr. Функция возвращает значение, или -1 при неудаче.
